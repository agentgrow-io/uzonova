...
#
# BLOG_BASE is the repo's GitHub Pages origin and is exported so
# regenerate_indexes.py + submit_indexing.py emit the correct canonical URLs.
# The provisioner renders the uzonova placeholder below per client.
# =============================================================================
...
export BLOG_BASE="${BLOG_BASE:-https://agentgrow-io.github.io/uzonova}"
...
# Caller exports APPROVAL_ID before invoking publish.sh; we only close the row
# if an actual commit landed.
...
if [ ${#URLS[@]} -eq 0 ]; then
    ...
    echo ""
    echo "4️⃣  Submitting to IndexNow + Google Indexing API (skipped if no key)..."
